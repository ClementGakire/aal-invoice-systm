  ...
  try {
    switch (request.method) {
      ...
      case 'POST':
        ...
        if (!newClientId || !jobData.jobNumber || !jobData.title || !jobData.jobType) {
          return response.status(400).json({ 
            error: 'Missing required fields: clientId, jobNumber, title, jobType' 
          });
        }
      ...
      case 'PUT':
        ...
        if (!updateId) {
          return response.status(400).json({ error: 'Job ID is required for updates' });
        }
      ...
      case 'DELETE':
        ...
        if (!deleteId) {
          return response.status(400).json({ error: 'Job ID is required for deletion' });
        }
